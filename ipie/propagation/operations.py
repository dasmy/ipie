...
def kinetic_real(phi, system, bt2, H1diag=False):
    ...
    if (H1diag):
        phi[:,:nup] = nl.einsum("ii,ij->ij", bt2[0],phi[:,:nup])
        phi[:,nup:] = nl.einsum("ii,ij->ij", bt2[1],phi[:,nup:])
    else:
...
def kinetic_spin_real_batch(phi, bt2, H1diag=False):
    ...
    # Assuming that our walker is in UHF form.
    if H1diag:
        phi[:,:] = nl.einsum("ii,wij->ij", bt2,phi)
    else:
        if is_cupy(bt2):
            phi = nl.einsum('ik,wkj->wij', bt2, phi, optimize=True)
        else:
